...
export default function Sidebar({ sidebarOpen, onToggleSidebar }) {
  ...
  /* -------------------------
       ...
       - RECTIFICATION: Fixed width [240px] and [72px] applied to match App.jsx margins
       ------------------------- */
  const DesktopSidebar = isHomePage ? (
    <aside
      className={`app-sidebar bg-white border-r p-4 fixed left-0 top-[var(--header-height)] h-[calc(100vh-var(--header-height))] z-30 ${
    ...
    >
      <div className="space-y-3">
        <Link
          to="/"
          className={`flex items-center gap-4 px-2 py-3 bg-gray-100 rounded-lg hover:bg-gray-100 ${!expanded ? 'justify-center' : ''}`}
        >
        ...
        {topItems.map((it) => (
          <Link
            key={it.key}
            to={it.to}
            className={`flex items-center gap-4 px-2 py-3 rounded-lg hover:bg-gray-50 ${!expanded ? 'justify-center' : ''}`}
          >
  ...
  /* -------------------------
       ...
       - RECTIFICATION: Standardized at [72px] to eliminate white space gap
       ------------------------- */
  const MediumCompactRail =
    isHomePage && !expanded ? (
      // Only visible on Home page, when collapsed (MD/LG screens)
      <div className="fixed top-[var(--header-height)] left-0 z-30 flex-col items-center hidden h-[calc(100vh-var(--header-height))] px-1 py-3 bg-white border-r md:flex xl:hidden w-[72px] overflow-y-auto">
        <Link
          to="/"
  ...
  const UniversalDrawer = expanded ? (
    ...
    >
      ...
      <div className="fixed left-0 z-50 max-w-full overflow-y-auto bg-white shadow-xl w-[240px] top-[var(--header-height)] h-[calc(100vh-var(--header-height))]">
        <DrawerContent />
      </div>
    </div>
  ) : null;
